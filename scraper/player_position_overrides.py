...
from football_bot.models import PlayerPosition


RAW_POSITION_TO_PLAYER_POSITION = {
    "Вратарь": PlayerPosition.GOALKEEPER,
    "Защитник": PlayerPosition.DEFENDER,
    "Оборонительный полузащитник": PlayerPosition.DEFENSIVE_MIDFIELDER,
    "Атакующий полузащитник": PlayerPosition.ATTACKING_MIDFIELDER,
    "Нападающий": PlayerPosition.FORWARD,
}

_FIXED_POSITION_ROWS = """
...
FIXED_PLAYER_POSITIONS_BY_EXTERNAL_ID: dict[str, PlayerPosition] = {
    external_id: PlayerPosition(position)
    for external_id, position in (
        row.split(",", 1) for row in _FIXED_POSITION_ROWS.splitlines()
    )
}


def get_fixed_player_position(external_id: str) -> PlayerPosition | None:
    return FIXED_PLAYER_POSITIONS_BY_EXTERNAL_ID.get(external_id)
